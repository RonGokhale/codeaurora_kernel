...
static void __init map_lowmem(void)
{
	...
	for_each_memblock(memory, reg) {
		...
		if (map.virtual <= (unsigned long)_stext &&
		    ((unsigned long)_end < (map.virtual + map.length))) {
			struct map_desc kernel;

			/* Kernel init. Set RW when freeing initmem. */
			kernel.pfn = __phys_to_pfn(__pa(__init_begin));
			kernel.virtual = (unsigned long)__init_begin;
			kernel.length = _sdata - __init_begin;
			kernel.type = MT_MEMORY_RWX;
			create_mapping(&kernel);

			/* Kernel text. Set RX when freeing initmem. */
			...
			/* Before the kernel, set RX when freeing initmem. */
			if (map.virtual < (unsigned long)_stext) {
				map.length = (unsigned long)_stext -
					     map.virtual;
				map.type = MT_MEMORY_RWX;
				create_mapping(&map);
			}

			/* Anything after the kernel. */
