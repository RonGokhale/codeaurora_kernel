...
#define PCM_BUFSZ_MIN 	8192	/* Hold one stereo WMA frame */
...
static int audio_enable(struct audio *audio)
{
	...
	if (msm_adsp_enable(audio->audplay)) {
		dprintk("audio: msm_adsp_enable(audplay) failed\n");
		audmgr_disable(&audio->audmgr);
		return -ENODEV;
	}

	if (audpp_enable(audio->dec_id, audio_dsp_event, audio)) {
		dprintk("audio: audpp_enable() failed\n");
			      ...
			      void (*getevent) (void *ptr, size_t len))
{
	...
	default:
		dprintk("unexpected message from decoder \n");
		break;
	}
}
...
static void audio_dsp_event(void *private, unsigned id, uint16_t *msg)
{
	...
	case AUDPP_MSG_STATUS_MSG:{
			...
			case AUDPP_DEC_STATUS_PLAY:
				...
				break;
			default:
				dprintk("unknown decoder status \n");
			}
			break;
		}
	case AUDPP_MSG_CFG_MSG:
		if (msg[0] == AUDPP_MSG_ENA_ENA) {
			...
			dprintk("audio_dsp_event: CFG_MSG %d?\n", msg[0]);
		}
		break;
	...
	default:
		dprintk("audio_dsp_event: UNKNOWN (%d)\n", id);
	}
...
static void audplay_buffer_refresh(struct audio *audio)
{
	...
	dprintk("audplay_buffer_refresh: buf0_addr=%x buf0_len=%d\n",
			...
			if ((config.pcm_feedback) && (!audio->read_data)) {
				...
				audio->read_data =
				    dma_alloc_coherent(NULL,
						       ...
						       GFP_KERNEL);
				if (!audio->read_data) {
					dprintk("audio_wma: buf alloc fail\n");
					rc = -ENOMEM;
				} else {
					uint8_t index;
					uint32_t offset = 0;
	...
	while (count > 0) {
		...
		if (count < audio->in[audio->read_next].used) {
			...
			if (copy_to_user
				...
				dprintk("audio_read: invalid addr %x \n",
				       (unsigned int)buf);
				rc = -EFAULT;
				break;
			}
...
static int audwma_handle_eos(struct audio *audio)
{
	int rc = 0;
	struct buffer *frame;
	char *buf_ptr;
			   ...
			   size_t count, loff_t *pos)
{
	...
	if (!count) { /* client signal EOS */
		return audwma_handle_eos(audio);
	}
...
static struct audio the_wma_audio;
...
static int audio_open(struct inode *inode, struct file *file)
{
	...
	while (pmem_sz >= DMASZ_MIN) {
		...
		if (audio->data)
			...
			dprintk("audio: could not allocate DMA buffers\n");
			rc = -ENOMEM;
			goto done;
		} else
			pmem_sz >>= 1;
	...
	if (rc) {
		dma_free_coherent(NULL, pmem_sz, audio->data, audio->phys);
		goto done;
	}
	...
	if (rc) {
		dprintk("audio: failed to get audplay0 dsp module\n");
...
static struct file_operations audio_wma_fops = {
	.owner		= THIS_MODULE,
	.open		= audio_open,
...
static struct file_operations audwma_debug_fops = {
...
static int __init audio_init(void)
{
	...
	mutex_init(&the_wma_audio.lock);
	mutex_init(&the_wma_audio.write_lock);
	mutex_init(&the_wma_audio.read_lock);
	mutex_init(&the_wma_audio.get_event_lock);
	...
	the_wma_audio.read_data = NULL;

#ifdef CONFIG_DEBUG_FS
	dentry = debugfs_create_file("msm_wma", S_IFREG | S_IRUGO, NULL,
		(void *) &the_wma_audio, &audwma_debug_fops);

	if (IS_ERR(dentry))
		dprintk("Wma:%s:debugfs_create_file failed\n", __func__);
#endif
