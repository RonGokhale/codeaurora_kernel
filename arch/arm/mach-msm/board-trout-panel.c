...
static void trout_set_backlight_level(uint8_t level)
{
	if (trout_new_backlight) {
		...
		if(level) {
			clk_enable(gp_clk);
			writel((1U << 16) | (~level & 0xffff), MSM_CLK_CTL_BASE + 0x58);
			if(level == 255) { /* Going directly to a 100% duty cycle does not seem to work */
				writel((~127 << 16) | 0xb20, MSM_CLK_CTL_BASE + 0x5c);
				udelay(1);
			}
...
static void trout_process_mddi_table(struct mddi_panel_info *panel, struct mddi_table *table, size_t count)
{
	int i;
	for(i = 0; i < count; i++) {
		...
		if (reg == 0)
			...
			mddi_remote_write(panel->mddi, value, reg);
	}
}
...
static void trout_mddi_client_power(int on)
{
    unsigned id, on_off;
	if(on) {
		on_off = 0;
...
static void trout_mddi_enable(struct mddi_panel_info *panel, int on)
{
	int panel_id;

	if(on) {
		mddi_set_auto_hibernate(panel->mddi, 0);
		trout_process_mddi_table(panel, mddi_toshiba_init_table,
					 ARRAY_SIZE(mddi_toshiba_init_table));
		mddi_set_auto_hibernate(panel->mddi, 1);
		panel_id = (mddi_remote_read(panel->mddi, GPIODATA) >> 4) & 3;
		if (panel_id > 1) {
...
static void trout_panel_power(struct mddi_panel_info *panel, int on)
{
	int panel_id;
	...
	if(on) {
		...
		mddi_set_auto_hibernate(panel->mddi, 0);
		trout_process_mddi_table(panel, mddi_toshiba_panel_init_table,
		...
		switch(panel_id) {
			case 0:
				...
				trout_process_mddi_table(panel,
		...
		mddi_set_auto_hibernate(panel->mddi, 1);
		// reenable vsync
		mddi_remote_write(panel->mddi, GPIOSEL_VWAKEINT, GPIOSEL);
		mddi_remote_write(panel->mddi, INTMASK_VWAKEOUT, INTMASK);
	} else {
		panel_id = (mddi_remote_read(panel->mddi, GPIODATA) >> 4) & 3;
		mddi_set_auto_hibernate(panel->mddi, 0);
		switch(panel_id) {
...
static int __init trout_backlight_init(void)
{
	return platform_driver_register(&trout_backlight_driver);
}

static void __exit trout_backlight_exit(void)
{
	platform_driver_unregister(&trout_backlight_driver);
}

static struct msm_mddi_platform_data msm_mddi0_pdata = {
	.mddi_client_power = trout_mddi_client_power,
	.panel_power = trout_panel_power,
	.mddi_enable = trout_mddi_enable,
	.has_vsync_irq = 1,
	.fb_base = MSM_FB_BASE,
	.fb_size = MSM_FB_SIZE,
};
...
static struct platform_device msm_mddi0_device = {
	.name = "msm_mddi",
	.id = 0,
	.dev = { .platform_data = &msm_mddi0_pdata },
};
...
int __init trout_init_panel(unsigned int sys_rev)
{
	int rc;
	...
	trout_new_backlight = sys_rev >= 5;
	if (trout_new_backlight) {
		uint32_t config = PCOM_GPIO_CFG(27, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_8MA); /* GPIO27 */
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
	}
	else {
		uint32_t config = PCOM_GPIO_CFG(27, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_8MA); /* GP_CLK */
		msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);

		gp_clk = clk_get(NULL, "gp_clk");
		if (IS_ERR(gp_clk)) {
			printk(KERN_ERR "trout_init_panel: could not get gp clock\n");
			gp_clk = NULL;
		}
		rc = clk_set_rate(gp_clk, 19200000);
		if (rc)
			printk(KERN_ERR "trout_init_panel: set clock rate failed\n");
	}

	rc = platform_device_register(&msm_mddi0_device);
	if (rc)
		return rc;
	return platform_device_register(&trout_backlight);
}

module_init(trout_backlight_init);
module_exit(trout_backlight_exit);
