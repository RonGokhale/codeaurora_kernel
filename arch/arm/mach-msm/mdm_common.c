...
struct mdm_device {
	...
	struct subsys_data mdm_subsys_data;
...
{
	unsigned long flags;
	struct mdm_device *lmdev;

	spin_lock_irqsave(&mdm_devices_lock, flags);
	list_for_each_entry(lmdev, &mdm_devices, link) {
		if (mdev && mdev == lmdev) {
...
static void mdm_start_ssr(struct mdm_device *mdev)
{
	...
	if (start_ssr) {
		...
		pr_info("%s: Resetting mdm id %d due to mdm error\n",
				__func__, mdev->mdm_data.device_id);
		subsystem_restart(mdev->subsys_name);
	} else {
				...
				unsigned long arg)
{
	...
	if (!filp || !filp->f_path.dentry->d_iname) {
		pr_err("%s: invalid filp\n", __func__);
		return -EINVAL;
	}
...
{
	struct mdm_device *mdev =
	 container_of(crashed_subsys, struct mdm_device, mdm_subsys_data);
	struct mdm_modem_drv *mdm_drv = &mdev->mdm_data;
						...
						struct mdm_device *mdev)
{
	...
	memset((void *)&mdev->mdm_subsys_data, 0,
		   sizeof(struct subsys_data));
	...
	mdev->mdm_subsys_data.shutdown = mdm_subsys_shutdown;
	mdev->mdm_subsys_data.ramdump = mdm_subsys_ramdumps;
	mdev->mdm_subsys_data.powerup = mdm_subsys_powerup;
	mdev->mdm_subsys_data.name = mdev->subsys_name;
...
static int mdm_configure_ipc(struct mdm_device *mdev)
{
	...
	ssr_register_subsystem(&mdev->mdm_subsys_data);
