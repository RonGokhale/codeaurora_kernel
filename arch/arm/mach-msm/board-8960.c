	...
	if (!pmem_param_set) {
		...
		if (machine_is_msm8960_liquid() ||
			...
			for (i = 0; i < msm8960_ion_pdata.nr; i++) {
				if (msm8960_ion_pdata.heaps[i].id == ION_SF_HEAP_ID) {
...
static void __init reserve_ion_memory(void)
{
	...
	/*
	 * The space for the firmware must be explicitly removed
	 */
	BUG_ON(memblock_remove(MSM8960_FW_START, MSM_MM_FW_SIZE));
	...
	if (low_use_cma) {
		...
		BUG_ON(!IS_ALIGNED(fixed_low_size, cma_alignment));
	} else {
		BUG_ON(!IS_ALIGNED(fixed_low_size, SECTION_SIZE));
	...
	if (middle_use_cma) {
		/*
		 * The middle heap needs to be aligned to work properly
		 * with CMA but the space between the low and middle
		 * heaps cannot exist in the kernel memory map.
		 * It must be removed.
		 */
		int want = round_up(fixed_middle_start, cma_alignment);
		BUG_ON(memblock_remove(fixed_middle_start, want - fixed_middle_start));
		fixed_middle_start = want;
