...
static struct msm_pcie_vreg_info_t msm_pcie_vreg_info[MSM_PCIE_MAX_VREG] = {
	{NULL, "PCIe01_3.3",	   0,       0,	     0},
	{NULL, "PCIe01_1.8", 1800000, 1800000, 1800000},
	{NULL, "PCIe01_0.9", 1000000, 1000000, 1000000}
};
...
static void msm_pcie_vreg_deinit(u32 rc_idx)
{
	...
	for (i = msm_pcie_dev[rc_idx].vreg_n - 1; i >= 0; i--) {
		...
		regulator_disable(msm_pcie_dev[rc_idx].vreg[i].hdl);
		msm_pcie_dev[rc_idx].vreg[i].hdl = NULL;
	}
}
...
static int msm_pcie_clk_init(u32 rc_idx, struct device *dev)
{
	...
	for (i = 0; i < MSM_PCIE_MAX_CLK; i++) {
		...
		while ((IS_ERR(clk_hdl) == -EPROBE_DEFER) && (j++ < 30)) {
			PCIE_DBG("got EPROBE_DEFER for clk %s for %d times\n",
					info->name, j);
		...
		if (!clk_hdl || IS_ERR(clk_hdl)) {
			...
			pr_err("PCIe: can't get clk %s: %d\n", info->name, rc);
			break;
		}

		if (info->freq) {
			rc = clk_set_rate(clk_hdl, info->freq);
			if (rc) {
	...
	if (rc) {
		...
		while (i--) {
			...
			clk_put(msm_pcie_dev[rc_idx].clk[i].hdl);
			msm_pcie_dev[rc_idx].clk[i].hdl = NULL;
		}
...
static int msm_pcie_get_resources(u32 rc_idx, struct platform_device *pdev)
{
	...
	dev->gpio_n = of_gpio_count((&pdev->dev)->of_node);

	for (i = 0; i < dev->gpio_n; i++) {
		gpio_info = &dev->gpio[i];
		gpio_info->num = of_get_gpio((&pdev->dev)->of_node, i);
	...
	for (i = 0; i < MSM_PCIE_MAX_RES; i++) {
		...
		res_info->base = ioremap(res->start, resource_size(res));
		if (!res_info->base) {
...
static void msm_pcie_release_resources(u32 rc_idx)
{
	...
	for (i = 0; i < MSM_PCIE_MAX_RES; i++) {
		iounmap(msm_pcie_dev[rc_idx].res[i].base);
		msm_pcie_dev[rc_idx].res[i].base = NULL;
	}
...
static int msm_pcie_enable(u32 rc_idx, u32 options)
{
	...
	/*
	gpio_direction_output(dev->gpio[MSM_PCIE_GPIO_PERST].num,
	...
	*/
	...
	/* init PCIe PHY */
	pcie_phy_init(rc_idx);

	PCIE_DBG("waiting for phy ready...\n");

	do {
		if (pcie_phy_is_ready(rc_idx))
			break;
		retries++;
		usleep_range(1000, 1500);
	} while (retries < 0xFFFFFFull);
