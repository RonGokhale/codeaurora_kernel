...
static long dsi_pll_vco_round_rate(struct clk *c, unsigned long rate)
{
	...
	if (rate < vco_clk->vco_clk_min / 4)
		factor = 8;
	else if (rate < vco_clk->vco_clk_min / 2)
		factor = 4;
	else if (rate < vco_clk->vco_clk_min)
		factor = 2;

	vco_rate = rate * factor;
	if (vco_rate < vco_clk->vco_clk_min)
		vco_rate = vco_clk->vco_clk_min;
	else if (vco_rate > vco_clk->vco_clk_max)
...
static int dsi_pll_byteclk_set_rate(struct clk *c, unsigned long rate)
{
	int i, ret;
	long vco_rate;
	...
	bitclk_rate = rate * 8;
	for (i = 1; i < 256; i++) {
		vco_rate = clk_round_rate(parent, bitclk_rate * i);

		if (vco_rate == bitclk_rate * i)
			break;
	...
	val = (temp & 0xFFFFFFF0) | ((vco_rate / bitclk_rate) - 1);
...
static long dsi_pll_byteclk_round_rate(struct clk *c, unsigned long rate)
{
	long rounded_rate, vco_rate;
	struct clk *parent = clk_get_parent(c);

	if (rate == 0)
		return -EINVAL;

	vco_rate = clk_round_rate(parent, rate * 8);

	rounded_rate = vco_rate % rate;
	if (rounded_rate == 0)
		rounded_rate = rate;
	else
		rounded_rate = vco_rate / (vco_rate - rounded_rate) * rate;

	return rounded_rate;
}

static enum handoff dsi_pll_byteclk_handoff(struct clk *c)
{
	struct clk *parent = clk_get_parent(c);
...
static int dsi_pll_dsiclk_set_rate(struct clk *c, unsigned long rate)
{
	u32 temp, val;
	int ret;
	struct clk *parent = clk_get_parent(c);
...
static long dsi_pll_dsiclk_round_rate(struct clk *c, unsigned long rate)
{
...
static enum handoff dsi_pll_dsiclk_handoff(struct clk *c)
{
	struct clk *parent = clk_get_parent(c);
	unsigned long vco_rate = clk_get_rate(parent);
	u32 out_div3;
...
int dsi_pll_prepare(struct clk *clk)
{
	return clk_prepare(dsi_ahb_clk);
}

void dsi_pll_unprepare(struct clk *clk)
{
...
struct clk_ops clk_ops_dsi_dsiclk = {
	...
	.set_rate = dsi_pll_dsiclk_set_rate,
	.round_rate = dsi_pll_dsiclk_round_rate,
	.handoff = dsi_pll_dsiclk_handoff,
};

struct clk_ops clk_ops_dsi_byteclk = {
	.prepare = dsi_pll_prepare,
	.unprepare = dsi_pll_unprepare,
	.set_rate = dsi_pll_byteclk_set_rate,
	.round_rate = dsi_pll_byteclk_round_rate,
	.handoff = dsi_pll_byteclk_handoff,
};
