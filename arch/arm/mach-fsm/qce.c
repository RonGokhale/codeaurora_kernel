...
/* qce operation maximum data size */
#define QCE_MAX_OPER_DATA      (32 * 1024)    /* 32 K */
		...
		uint32_t totallen, uint32_t coffset)
{
	...
	default:
		...
		default:
			...
			/* check for null key. If null key, use hw key */
		...
		if (aes_key_chg) {
			if (pce_dev->fastaes) {
				...
				for (i = 0; i < CRYPTO_AES_RNDKEYS; i++) {
...
static int _ce_f9_setup(struct qce_device *pce_dev, struct qce_f9_req * req)
{
	...
	writel(ikey[0], pce_dev->iobase + CRYPTO_AUTH_IV0_REG);
	writel(ikey[1], pce_dev->iobase + CRYPTO_AUTH_IV1_REG);
	writel(ikey[2], pce_dev->iobase + CRYPTO_AUTH_IV2_REG);
	writel(ikey[3], pce_dev->iobase + CRYPTO_AUTH_IV3_REG);
	writel(req->last_bits, pce_dev->iobase + CRYPTO_AUTH_IV4_REG);

	writel(req->fresh, pce_dev->iobase + CRYPTO_AUTH_BYTECNT0_REG);
	writel(req->count_i, pce_dev->iobase + CRYPTO_AUTH_BYTECNT1_REG);

	/* write auth_seg_cfg */
	writel((uint32_t)req->msize << CRYPTO_AUTH_SEG_SIZE,
			pce_dev->iobase + CRYPTO_AUTH_SEG_CFG_REG);
	...
	/* write seg_size   */
	writel(req->msize, pce_dev->iobase + CRYPTO_SEG_SIZE_REG);
		...
		uint16_t cipher_size)
{
	...
	/* write seg_size   */
	writel(req->data_len, pce_dev->iobase + CRYPTO_SEG_SIZE_REG);

	/* write 0 to auth_size, auth_offset */
	writel(0, pce_dev->iobase + CRYPTO_AUTH_SEG_CFG_REG);

	/* write encr_seg_cfg seg_size, seg_offset */
	writel((((uint32_t) cipher_size) << CRYPTO_ENCR_SEG_SIZE) |
			(cipher_offset & 0xffff),
				pce_dev->iobase + CRYPTO_ENCR_SEG_CFG_REG);

	/* write keys */
	_byte_stream_to_net_words(ckey, &req->ckey[0], OTA_KEY_SIZE);
	writel(ckey[0], pce_dev->iobase + CRYPTO_DES_KEY0_REG);
	writel(ckey[1], pce_dev->iobase + CRYPTO_DES_KEY1_REG);
	writel(ckey[2], pce_dev->iobase + CRYPTO_DES_KEY2_REG);
	writel(ckey[3], pce_dev->iobase + CRYPTO_DES_KEY3_REG);

	/* write cntr0_iv0 for countC */
	writel(req->count_c, pce_dev->iobase + CRYPTO_CNTR0_IV0_REG);

	/* write cntr1_iv1 for nPkts, and bearer */
	if (npkts == 1)
		npkts = 0;
	writel(req->bearer << CRYPTO_CNTR1_IV1_REG_F8_BEARER |
...
int qce_ablk_cipher_req(void *handle, struct qce_req *q_req)
{
	int rc = 0;
	struct qce_device *pce_dev = (struct qce_device *) handle;
	...
	q_req->ivsize = crypto_ablkcipher_ivsize(tfm);
	q_req->cryptlen = areq->nbytes;
	...
	/* set up crypto device */
	rc = _ce_setup(pce_dev, q_req, areq->nbytes, 0);
	if (rc < 0)
		goto bad;
	...
	pce_dev->chan_ce_in_cmd->complete_func = _ablk_cipher_ce_in_call_back;
	pce_dev->chan_ce_out_cmd->complete_func = _ablk_cipher_ce_out_call_back;
