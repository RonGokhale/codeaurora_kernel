...
static int ftr_ioctl(struct inode *inode, struct file *file,
	...
	case RFIC_IOCTL_READ_REGISTER:
		{
			...
			down(&pdev->sem);
			writel(pdev->busSelect[RFIC_FTR_GET_BUS(rficAddr)],
				pdev->grfcCtrlAddr);
			writel(pdev->busSelect[RFIC_FTR_GET_BUS(rficAddr)],
				pdev->grfcCtrlAddr);
	...
	case RFIC_IOCTL_GET_GRFC:
		{
			...
			param.maskValue = readl(
				MSM_GRFC_BASE + 0x18 + param.grfcId * 4);
			param.ctrlValue = readl(
	...
	case RFIC_IOCTL_SET_GRFC:
		{
			...
			writel(param.maskValue,
				MSM_GRFC_BASE + 0x18 + param.grfcId * 4);
			writel(param.ctrlValue,
				MSM_GRFC_BASE + 0x00 + param.grfcId * 4);
			writel(param.ctrlValue,
				MSM_GRFC_BASE + 0x00 + param.grfcId * 4);
		}
...
static int __init ftr_init(void)
{
	...
	for (i = 0; i < RFIC_FTR_DEVICE_NUM; ++i) {
		...
		if (i == 0) {
			...
			writel(0x300000, pdev->grfcMaskAddr);
			...
			writel(0x480000, pdev->grfcMaskAddr);
			pdev->busSelect[TX1_BUS] = 0x000000;
