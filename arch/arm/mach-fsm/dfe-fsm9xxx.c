...
static int hh_ioctl(struct inode *inode, struct file *file,
	unsigned int cmd, unsigned long arg)
{
	...
	case DFE_IOCTL_IS_UMTS:
		return readl(MSM_TCSR_BASE + 0x0008) & 0x01;
	...
	case DFE_IOCTL_READ_REGISTER:
		{
			...
			if (!HH_OFFSET_VALID(offset))
				return -EINVAL;
			value = readl(HH_REG_IOADDR(offset));
	...
	case DFE_IOCTL_WRITE_REGISTER:
		{
			...
			if (!HH_OFFSET_VALID(param.offset))
				return -EINVAL;
			writel(param.value, HH_REG_IOADDR(param.offset));
		}
		break;
	...
	case DFE_IOCTL_WRITE_REGISTER_WITH_MASK:
		{
			...
			spin_lock_irqsave(&hh_dev_info.hh_lock,
				irq_flags);
			value = readl(HH_REG_IOADDR(param.offset));
			value &= ~param.mask;
			value |= param.value & param.mask;
			writel(value, HH_REG_IOADDR(param.offset));
	...
	case DFE_IOCTL_WRITE_REGISTER_ARRAY:
		{
			...
			for (i = 0; i < param.num; ++i, addr += 4) {
				if (cmd == DFE_IOCTL_READ_REGISTER_ARRAY)
					pdfi->parray[i] = readl(addr);
				else
					writel(pdfi->parray[i], addr);
			}
			spin_unlock_irqrestore(&hh_dev_info.hh_lock,
				irq_flags);
...
static irqreturn_t hh_irq_handler(int irq, void *data)
{
	...
	irq_enable = readl(HH_REG_SCPN_IREQ_MASK);
	irq_flag = readl(HH_REG_SCPN_IREQ_FLAG);
	irq_flag &= irq_enable;

	/* Disables interrupts */
	irq_enable &= ~irq_flag;
	writel(irq_enable, HH_REG_SCPN_IREQ_MASK);
...
static int __init hh_init(void)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "Cannot register HH interrupt handler.\n");
		return ret;
	}
