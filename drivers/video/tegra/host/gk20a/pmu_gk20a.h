...
struct pmu_ucode_desc {
	...
	u32 app_resident_code_size;    /* Exact size of the resident code
					  (potentially contains CRC inside at
					   the end) */
	u32 app_resident_data_offset;  /* Offset from appStartOffset */
	u32 app_resident_data_size;    /* Exact size of the resident code
					  (potentially contains CRC inside at
					   the end) */
	u32 nb_overlays;
	struct {
		u32 start;
		u32 size;
	} load_ovl[GK20A_PMU_UCODE_NB_MAX_OVERLAY];
	u32 compressed;
};
...
struct pmu_queue {
	...
	bool locked; /* check free space after setting locked but before
			setting opened */
};
...
typedef void (*pmu_callback)(struct gk20a *, struct pmu_msg *, void *,
			     u32, u32);
...
#define PMU_ELPG_STAT_ON_PENDING	2   /* elpg is off, ALLOW cmd has been
					       sent, wait for ack */
#define PMU_ELPG_STAT_OFF_PENDING	3   /* elpg is on, DISALLOW cmd has
					       been sent, wait for ack */
#define PMU_ELPG_STAT_OFF_ON_PENDING	4   /* elpg is off, caller has
					       requested on, but ALLOW
					       cmd hasn't been sent due to
					       ENABLE_ALLOW delay */
...
#define PMU_ELPG_ENABLE_ALLOW_DELAY_MSEC	1 /* msec */
	/* deferred elpg enable */
	struct delayed_work elpg_enable; /* deferred elpg enable */
	/* true after init, false after disable, true after delay */
	bool elpg_enable_allow;
...
int gk20a_pmu_cmd_post(struct gk20a *g, struct pmu_cmd *cmd,
		struct pmu_msg *msg, struct pmu_payload *payload, u32 queue_id,
		pmu_callback callback, void *cb_param, u32 *seq_desc,
