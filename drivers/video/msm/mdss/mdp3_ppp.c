...
struct ppp_status {
	...
	struct semaphore mdp3_ppp_mutex;
};
...
int mdp3_ppp_verify_res(struct mdp_blit_req *req)
{
	...
	if (((req->src_rect.x + req->src_rect.w) > req->src.width) ||
		...
		pr_err("%s: src roi larger than boundry\n", __func__);
		return -EINVAL;
	}

	if (((req->dst_rect.x + req->dst_rect.w) > req->dst.width) ||
	    ((req->dst_rect.y + req->dst_rect.h) > req->dst.height)) {
		pr_err("%s: dst roi larger than boundry\n", __func__);
		return -EINVAL;
	}
	return 0;
}
...
uint32_t mdp3_calc_tpval(struct ppp_img_desc *img, uint32_t old_tp)
{
	...
	tpVal = 0;
	if ((img->colorFormat == MDP_RGB_565)
	    || (img->colorFormat == MDP_BGR_565)) {
...
void mdp3_start_ppp(struct ppp_blit_op *blit_op)
{
	...
	down(&ppp_stat->mdp3_ppp_mutex);
	config_ppp_op_mode(blit_op);
	mdp3_ppp_kickoff();
	up(&ppp_stat->mdp3_ppp_mutex);
}
	...
	struct mdp3_img_data *dst_data)
{
	...
	blit_op->dst.colorFormat = req->dst.format;
	...
	blit_op->src.colorFormat = req->src.format;
	...
	if (req->flags & MDP_IS_FG)
		blit_op->mdpOp |= MDPOP_LAYER_IS_FG;

	/* blending check */
	if (req->transp_mask != MDP_TRANSP_NOP) {
		blit_op->mdpOp |= MDPOP_TRANSP;
		blit_op->blend.transColor =
			mdp3_calc_tpval(&blit_op->src, req->transp_mask);
	} else {
		blit_op->blend.transColor = 0;
	}

	req->alpha &= 0xff;
	if (req->alpha < MDP_ALPHA_NOP) {
		blit_op->mdpOp |= MDPOP_ALPHAB;
		blit_op->blend.constAlpha = req->alpha;
	} else {
	...
	if (req->flags & MDP_FLIP_LR)
		blit_op->mdpOp |= MDPOP_LR;
	if (req->flags & MDP_FLIP_UD)
		blit_op->mdpOp |= MDPOP_UD;
	if (req->flags & MDP_ROT_90)
		blit_op->mdpOp |= MDPOP_ROT90;
	if (req->flags & MDP_DITHER)
		blit_op->mdpOp |= MDPOP_DITHER;

	if (req->flags & MDP_BLEND_FG_PREMULT)
		blit_op->mdpOp |= MDPOP_FG_PM_ALPHA;
	...
	if (req->flags & MDP_BLUR)
		blit_op->mdpOp |= MDPOP_ASCALE | MDPOP_BLUR;
}
	...
	struct mdp3_img_data *dst_data)
{
	...
	if (((blit_op.mdpOp & (MDPOP_TRANSP | MDPOP_ALPHAB)) ||
	     (req->src.format == MDP_ARGB_8888) ||
	     (req->src.format == MDP_BGRA_8888) ||
	     (req->src.format == MDP_RGBA_8888)) &&
	    (blit_op.mdpOp & MDPOP_ROT90) && (req->dst_rect.w <= 16)) {
...
int mdp3_ppp_init(void)
{
	...
	sema_init(&ppp_stat->mdp3_ppp_mutex, 1);
	ppp_stat->busy = false;
