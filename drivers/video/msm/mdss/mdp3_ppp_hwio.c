	...
	if (k3 == one) {
		...
		if (!rpa_on) {
			...
			if (!org) {
				...
				/* calc end coord in dest space then map to
				   source space */
				Ed = Odprime +
				    ((uint64_t) dim_out << PQF_PLUS_4) - one;
				Es = (k1 * (Ed >> PQF_PLUS_4)) + k2;

				/* now floor & inc by 2 to calc the required
				   ending coord */
				...
				beg_of = org - (uint32_t) (Oreq >> PQF_PLUS_4);
				end_of =
				    (uint32_t) (Ereq >> PQF_PLUS_4) - (org +
								     dim_in -
								     1);
			}
		} else {
			...
			/* calculate end coord in destination space then map to
			   source space */
			Ed = Odprime + ((uint64_t) dim_out << PQF_PLUS_4) - one;
	...
	uint8_t *addr, uint32_t bpp, uint32_t uv)
{
	...
	if (uv && ((iBuf->dst.colorFormat == MDP_Y_CBCR_H2V2) ||
		(iBuf->dst.colorFormat == MDP_Y_CRCB_H2V2)))
		h_slice = 2;

	if (((iBuf->mdpOp & MDPOP_ROT90) == MDPOP_ROT90) ^
		((iBuf->mdpOp & MDPOP_LR) == MDPOP_LR)) {
		addr +=
		    (iBuf->dst.roi.width -
			    MIN(16, iBuf->dst.roi.width)) * bpp;
	}
	if ((iBuf->mdpOp & MDPOP_UD) == MDPOP_UD) {
		if (1) {
			addr +=
				((iBuf->dst.roi.height -
				MIN(16, iBuf->dst.roi.height))/h_slice) * dest_ystride;
		} else {
			addr +=
			(iBuf->dst.roi.width -
				MIN(16, iBuf->dst.roi.width)) * bpp;
...
{
	uint32_t bpp = bytes_per_pixel(img->colorFormat);
	int x = img->roi.x;
	int y = img->roi.y;
	uint32_t width = img->prop.width;

	if (img->colorFormat == MDP_Y_CBCR_H2V2_ADRENO && layer == 0)
	...
	if (img->p1) {
		 ...
		 */
		if (img->colorFormat == MDP_Y_CBCR_H2V2_ADRENO
							&& layer == 0)
...
int config_ppp_background(struct ppp_img_desc *bg)
{
	...
	PPP_WRITEL(src_config_lut(bg->colorFormat),
		MDP3_PPP_BG_FORMAT);
	PPP_WRITEL(pack_pattern_lut(bg->colorFormat),
		MDP3_PPP_BG_UNPACK_PATTERN1);
	return 0;
}

void ppp_edge_rep_luma_pixel(struct ppp_blit_op *blit_op,
	struct mdp3_ppp_edge_rep *er)
{
	if (blit_op->mdpOp & MDPOP_ASCALE) {

		er->is_scale_enabled = 1;

		if (blit_op->mdpOp & MDPOP_ROT90) {
	...
	struct mdp3_ppp_edge_rep *er)
{
	...
	switch (blit_op->src.colorFormat) {
	case MDP_Y_CBCR_H2V1:
	case MDP_Y_CRCB_H2V1:
...
int config_ppp_bg_edge_rep(struct ppp_blit_op *blit_op)
{
	...
	switch (blit_op->dst.colorFormat) {
	case MDP_Y_CBCR_H2V2:
	case MDP_Y_CRCB_H2V2:
...
int config_ppp_scale(struct ppp_blit_op *blit_op, uint32_t *pppop_reg_ptr)
{
	...
	if (blit_op->mdpOp & MDPOP_ASCALE) {
		...
		*pppop_reg_ptr |=
		...
		mdp_blur = blit_op->mdpOp & MDPOP_BLUR;
			...
			uint32_t *pppop_reg_ptr)
{
	...
	uint32_t alpha, transColor;
	uint32_t val = 0;
	int c_fmt = blit_op->src.colorFormat;
	int bg_alpha;

	csc = csc_rgb2yuv();
	alpha = blit_op->blend.constAlpha;
	transColor = blit_op->blend.transColor;
	if (blit_op->mdpOp & MDPOP_FG_PM_ALPHA) {
		if (per_pix_alpha(c_fmt)) {
			*pppop_reg_ptr |= PPP_OP_ROT_ON |
					  PPP_OP_BLEND_ON |
					  PPP_OP_BLEND_CONSTANT_ALPHA;
		} else {
			if ((blit_op->mdpOp & MDPOP_ALPHAB)
				&& (blit_op->blend.constAlpha == 0xff)) {
				blit_op->mdpOp &= ~(MDPOP_ALPHAB);
			}

			if ((blit_op->mdpOp & MDPOP_ALPHAB)
			   || (blit_op->mdpOp & MDPOP_TRANSP)) {
		...
		if ((per_pix_alpha(c_fmt)) && !(blit_op->mdpOp &
						MDPOP_LAYER_IS_FG)) {
			bg_alpha |= PPP_BLEND_BG_SRCPIXEL_ALPHA;
		} else {
			bg_alpha |= PPP_BLEND_BG_CONSTANT_ALPHA;
			bg_alpha |= blit_op->blend.constAlpha << 24;
		}
		PPP_WRITEL(bg_alpha, MDP3_PPP_BLEND_BG_ALPHA_SEL);

		if (blit_op->mdpOp & MDPOP_TRANSP)
			*pppop_reg_ptr |= PPP_BLEND_CALPHA_TRNASP;
	} else if (per_pix_alpha(c_fmt)) {
		if (blit_op->mdpOp & MDPOP_LAYER_IS_FG)
			*pppop_reg_ptr |= PPP_OP_ROT_ON |
				  PPP_OP_BLEND_ON |
				  PPP_OP_BLEND_CONSTANT_ALPHA;
		else
	...
	if (*pppop_reg_ptr & PPP_OP_BLEND_ON) {
		...
		if (blit_op->dst.colorFormat == MDP_YCRYCB_H2V1) {
			*pppop_reg_ptr |= PPP_OP_BG_CHROMA_H2V1;
			if (blit_op->mdpOp & MDPOP_TRANSP) {
				transColor = conv_rgb2yuv(transColor,
					&csc->fwd_matrix[0],
					&csc->bv[0],
					&csc->lv[0]);
			}
	...
	val |= (transColor & MDP_BLEND_TRASP_COL_MASK);
	PPP_WRITEL(val, MDP3_PPP_BLEND_PARAM);
	return 0;
}

int config_ppp_rotation(uint32_t mdpOp, uint32_t *pppop_reg_ptr)
{
	*pppop_reg_ptr |= PPP_OP_ROT_ON;
...
int config_ppp_op_mode(struct ppp_blit_op *blit_op)
{
	...
	//TODO find right place to do this
	ppp_init_default_vals();

	ppp_operation_reg |= ppp_dst_op_reg(blit_op->dst.colorFormat);
	switch (blit_op->dst.colorFormat) {
	case MDP_Y_CBCR_H2V2:
	case MDP_Y_CRCB_H2V2:
	...
	ppp_operation_reg |= ppp_src_op_reg(blit_op->src.colorFormat);
	switch (blit_op->src.colorFormat) {
	case MDP_Y_CBCR_H2V2:
	case MDP_Y_CBCR_H2V2_ADRENO:
	case MDP_Y_CRCB_H2V2:
	...
	config_ppp_csc(blit_op->src.colorFormat,
		blit_op->dst.colorFormat, &ppp_operation_reg);

	if (blit_op->mdpOp & MDPOP_DITHER)
		ppp_operation_reg |= PPP_OP_DITHER_EN;

	if (blit_op->mdpOp & MDPOP_ROTATION)
		config_ppp_rotation(blit_op->mdpOp, &ppp_operation_reg);

	if (blit_op->src.colorFormat == MDP_Y_CBCR_H2V2_ADRENO) {
		blit_op->src.stride0 = ALIGN(blit_op->src.prop.width, 32) *
			bytes_per_pixel(blit_op->src.colorFormat);
	...
	blit_op->dst.stride0 = blit_op->dst.prop.width *
		bytes_per_pixel(blit_op->dst.colorFormat);

	if (ppp_multi_plane(blit_op->dst.colorFormat)) {
...
void ppp_init_default_vals(void)
{
	load_ppp_lut(LUT_PRE_TABLE, default_pre_lut());
	load_ppp_lut(LUT_POST_TABLE, default_post_lut());

	load_csc_matrix(CSC_PRIMARY_MATRIX, csc_rgb2yuv());
	load_csc_matrix(CSC_SECONDARY_MATRIX, csc_default_table2());
}
