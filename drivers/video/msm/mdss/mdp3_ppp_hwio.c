...
{
	uint32_t bpp = ppp_bpp(img->colorFormat);
	int x = img->roi.x;
	int y = img->roi.y;
...
int config_ppp_background(struct ppp_img_desc *bg)
{
	...
	PPP_WRITEL(ppp_src_config(bg->colorFormat),
		MDP3_PPP_BG_FORMAT);
	PPP_WRITEL(ppp_pack_pattern(bg->colorFormat),
			...
			uint32_t *pppop_reg_ptr)
{
	...
	if (blit_op->mdpOp & MDPOP_FG_PM_ALPHA) {
		...
		if ((ppp_per_p_alpha(c_fmt)) && !(blit_op->mdpOp &
						MDPOP_LAYER_IS_FG)) {
...
int config_ppp_op_mode(struct ppp_blit_op *blit_op)
{
	...
	if (blit_op->src.colorFormat == MDP_Y_CBCR_H2V2_ADRENO) {
		blit_op->src.stride0 = ALIGN(blit_op->src.prop.width, 32) *
			ppp_bpp(blit_op->src.colorFormat);
	...
	blit_op->dst.stride0 = blit_op->dst.prop.width *
		ppp_bpp(blit_op->dst.colorFormat);
