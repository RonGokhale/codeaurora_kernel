			...
			struct msm_fb_data_type *mfd)
{
	...
	if (req->src_rect.h > 0xFFF || req->src_rect.h < 3) {
	...
	if (req->src_rect.w > 0xFFF || req->src_rect.w < 3) {
	...
	if (req->dst_rect.h > 0xFFF || req->dst_rect.h < 3) {
	...
	if (req->dst_rect.w > 0xFFF || req->dst_rect.w < 3) {
