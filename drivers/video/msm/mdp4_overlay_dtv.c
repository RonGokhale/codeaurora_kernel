...
int mdp4_dtv_off(struct platform_device *pdev)
{
	...
	/*
	 clean up ion freelist there need two stage
	 to empty ion free list therefore need call
	 unmap freelist twice      */
	mdp4_overlay_iommu_unmap_freelist(mixer);
	mdp4_overlay_iommu_unmap_freelist(mixer);
...
static void mdp4_dtv_blt_dmae_update(struct mdp4_overlay_pipe *pipe)
{
	...
	case 3:
		...
		break;
	default:
		pr_err("%s error dmae_cnt can't be more than 3 ", __func__);
		break;
	}
...
static void mdp4_dtv_do_blt(struct msm_fb_data_type *mfd, int enable)
{
	...
	if (tg_enabled) {
		mdp4_dtv_wait4vsync(cndx, &vtime);
		MDP_OUTP(MDP_BASE + DTV_BASE, 0);
