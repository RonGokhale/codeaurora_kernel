	...
	for (i = 0; i < 5; i++) {
		...
		case 0x03:
			...
			if (((SINK_OUI[0] == 0x00) &&
				...
				sp_tx_rx_type = RX_DP;
				DEV_NOTICE("Downstream is DP2VGA converter.");
			}
			ds_port_recoginze = 1;
			break;
		case 0x05:
			...
			if ((SINK_OUI[0] == 0xb9) && (SINK_OUI[1] == 0x22)
				...
				sp_tx_rx_type = RX_DP;
				DEV_NOTICE("Downstream is DP2HDMI converter.");
			}
			ds_port_recoginze = 1;
			break;
		default:
...
static void sp_tx_polling_err_int_handler(void)
{
	...
	for (i = 0; i < 5; i++) {
		ok = sp_tx_aux_dpcdread_bytes(0x00, 0x00, 0x00, 1, &c);
		if (AUX_OK == ok)
			return;
	...
	if ((IRQ_Vector & SINK_SPECIFIC_IRQ) && (sp_tx_rx_type == RX_HDMI)) {
		...
		if (((Int_vector1 & 0x04) == 0x04) &&
			...
			if (((al & 0x01) == 0) || (sl_cr == 0)) {
				...
				if (sl_cr == 0)
					DEV_ERR("Lane clock not recovered\n");
		...
		if ((Int_vector1 & 0x08) == 0x08) {
			...
			if ((Int_vector1 & 0x10) != 0x10)
				...
				if (sp_tx_system_state > STATE_CONFIG_OUTPUT) {
					...
					sp_tx_clean_hdcp();
					sp_tx_set_sys_state(STATE_HDCP_AUTH);
					DEV_ERR("Downstream HDCP failure!");
				}
			}
		}
		...
		if ((Int_vector2 & 0x04) == 0x04) {
			...
			if ((c & 0x40) == 0x40)
				DEV_NOTICE("Downstream HDMI detected!\n");
		}
...
void sp_tx_hdcp_process(void)
{
	...
	if ((hdcp_en == 0) && (sp_tx_rx_type == RX_VGA)) {
