				      ...
				      unsigned long flags)
{
	...
	if (msm_use_iommu()) {
		...
		sg_init_table(data->iommu_sglist, num_large_pages);
		data->num_large_pages = num_large_pages;

		i = 0;
		sg = data->iommu_sglist;
...
static void ion_iommu_heap_free(struct ion_buffer *buffer)
{
	...
	for_each_sg(data->iommu_sglist, sg, data->num_large_pages, i)
