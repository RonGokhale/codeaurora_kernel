...
static void z180_dump_regs(struct kgsl_device *device)
{
	...
	KGSL_LOG_DUMP(device, "Z180 Register Dump Finished\n\n");
}
...
static void z180_dump_ringbuffer(struct kgsl_device *device)
{
	...
	KGSL_LOG_DUMP(device, "Z180 ringbuffer dump finished\n\n");
}
...
static void z180_dump_ib(struct kgsl_device *device)
{
	...
	if (z180_dev->current_timestamp == 0)
		KGSL_LOG_DUMP(device, "current_timestamp is zero,"
					" z180 has possibly not been"
					" used once since system boot\n");
	else {
		KGSL_LOG_DUMP(device, "rb_words: %d\n", rb_words);
		KGSL_LOG_DUMP(device, "Current IB's ringbuffer slot#: %d\n",
				z180_dev->current_timestamp % Z180_PACKET_COUNT);
		KGSL_LOG_DUMP(device, "Current timestamp: %d\n",
	...
	for (i = 0; i < rb_words; i++) {
		if (rb_hostptr[i] == Z180_STREAM_PACKET_CALL) {
			...
			entry = kgsl_get_mem_entry(device, pt_base, ib_gpuptr, 1);
