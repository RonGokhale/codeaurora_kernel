					...
					uint32_t flags)
{
	...
	if (link == NULL) {
		KGSL_CORE_ERR("kmalloc(%lu) failed\n", PAGE_SIZE);
		goto done;
	}
	...
	*cmds++ = 0x7fff;
	 ...
	 */
	result = adreno_ringbuffer_issuecmds(device, adreno_ctx, KGSL_CMD_FLAGS_PMODE,
...
adreno_probe(struct platform_device *pdev)
{
	...
	/* This is only for a3xx v2,
	 * hard code in here as prob doesn't have chipID information
	 */
