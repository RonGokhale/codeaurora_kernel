...
void iscsi_target_set_sock_callbacks(struct iscsi_conn *conn)
{
	struct sock *sk;
...
void iscsi_target_restore_sock_callbacks(struct iscsi_conn *conn)
{
	struct sock *sk;
...
static void iscsi_target_do_login_rx(struct work_struct *work)
{
	...
	read_lock_bh(&sk->sk_callback_lock);
	state = iscsi_target_sk_state_check(conn->sock->sk);
	read_unlock_bh(&sk->sk_callback_lock);
...
get_target:
	 ...
	 * in per-cpu-ida tag allocation logic + small se_nacl->queue_depth.
	 */
