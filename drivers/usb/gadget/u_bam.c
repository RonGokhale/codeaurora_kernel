...
#define CH_OPENED	BIT(0)
#define CH_READY	BIT(1)
struct bam_ch_info {
	unsigned long		flags;
	unsigned		id;
...
static int gbam_data_ch_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < n_ports; i++) {
		if (!strncmp(bam_ch_names[i], pdev->name,
			...
			clear_bit(CH_READY, &d->flags);
			clear_bit(CH_OPENED, &d->flags);
		}
	}
		...
		size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < n_ports; i++) {
		...
		temp += scnprintf(buf + temp, DEBUG_BUF_SIZE - temp,
				...
				d->tx_skb_q.qlen,
				test_bit(CH_OPENED, &d->flags),
				test_bit(CH_READY, &d->flags));
