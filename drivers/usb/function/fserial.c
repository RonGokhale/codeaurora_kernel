...
static unsigned int fs_buf_put(struct fs_buf *gb, const char *buf, unsigned int count);
static unsigned int fs_buf_get(struct fs_buf *gb, char *buf, unsigned int count);
...
static int fs_open(struct tty_struct *tty, struct file *file)
{
	...
	if (mutex_lock_interruptible(mtx)) {
		printk(KERN_ERR "fs_open: (%d,%p,%p) interrupted waiting for mutex\n",
			port_num, tty, file);
		return -ERESTARTSYS;
	}
	...
	if (ctxt->dev_config == FS_NO_CONFIG_ID) {
		printk(KERN_ERR "fs_open: (%d,%p,%p) device is not connected\n",
			port_num, tty, file);
		ret = -ENODEV;
...
static void fs_close(struct tty_struct *tty, struct file *file)
{
	...
	if (port->port_open_count == 0) {
		printk(KERN_ERR "fs_close: (%d,%p,%p) port is already closed\n",
			port->port_num, tty, file);
		goto exit;
	}
	...
	if (fs_buf_data_avail(port->port_write_buf) > 0) {
		...
		wait_event_interruptible_timeout(port->port_write_wait,
			FS_WRITE_FINISHED_EVENT_SAFELY(port), FS_CLOSE_TIMEOUT * HZ);
		spin_lock_irq(&port->port_lock);
	}
...
static void fs_flush_chars(struct tty_struct *tty)
{
	...
	if (port->port_dev == NULL) {
		printk(KERN_ERR "fs_flush_chars: (%d,%p) port is not connected\n",
			port->port_num, tty);
		goto exit;
	}
	...
	while (!list_empty(&ctxt->dev_write_list)) {
		...
		if (len > 0) {
			...
			if ((ret = usb_ept_queue_xfer(ep, req)) < 0) {
				printk(KERN_ERR "fs_send: cannot queue read request, ret=%d\n", ret);
				spin_lock_irqsave(&ctxt->dev_lock, flags);
				break;
			}
			spin_lock_irqsave(&ctxt->dev_lock, flags);
...
static int fs_send_packet(struct userial_context *ctxt, char *packet, unsigned int size)
{
	...
	if (port == NULL) {
		printk(KERN_ERR "fs_send_packet: port == NULL\n");
		return -EIO;
	}
...
static void fs_read_complete(struct usb_endpoint *ep, struct usb_request *req)
{
	...
	fs_debug_level(3, "fs_read complete data:%c %c %c.... len= %u\n",
		*((char *)req->buf), *((char *)req->buf+1), *((char *)req->buf+2), req->actual);
...
requeue:
		...
		if ((ret = usb_ept_queue_xfer(ctxt->dev_out_ep, req)) < 0) {
			printk(KERN_ERR "fs_read_complete: cannot queue read request, ret=%d\n", ret);
		}
		break;
	...
	default:
		/* unexpected */
		printk(KERN_ERR "fs_read_complete: unexpected status error, status=%d\n", req->status);
		goto requeue;
		break;
	}
}
...
static void fs_write_complete(struct usb_endpoint *ep, struct usb_request *req)
{
	...
	fs_debug_level(3, "fs_write_complete:%c %c %c len:%u\n", *((char *)req->buf),
			*((char *)req->buf+1), *((char *)req->buf+2), req->actual);
...
requeue:
		if (req == NULL) {
			printk(KERN_ERR "fs_write_complete: NULL request pointer\n");
			return;
		}
	...
	default:
		printk(KERN_ERR "fs_write_complete: unexpected status error, status=%d\n", req->status);
		goto requeue;
		break;
	}
}
 ...
 */
/* TODO: move disconnect function to fs_close? */
