	...
	enum msm_ispif_vfe_intf vfe_intf, int enable)
{
	...
	} else if (ispif->csid_version >= CSID_VERSION_V30) {
		if (vfe_intf == VFE0) {
...
static long msm_ispif_cmd(struct v4l2_subdev *sd, void *arg)
{
	...
	BUG_ON(!sd);
	BUG_ON(!pcdata);
pr_err("lok: %s: %d: cfg_type: %d  csid_ver: %d",__func__,__LINE__,pcdata->cfg_type, pcdata->csid_version);
pcdata->csid_version = 1;
	...
	case ISPIF_ENABLE_REG_DUMP:
		...
		break;
	case ISPIF_INIT:
		rc = msm_ispif_init(ispif, 1);//pcdata->csid_version);
