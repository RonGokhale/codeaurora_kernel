  ...
  struct platform_device *pdev)
{
	...
	for (i=0; i < count; i++) {
		rc = of_property_read_string_index(of_node, "qcom,clock-names", i,
			&(cci_clk_info[i].clk_name));
		CDBG("%s: clock-names[%d] = %s\n", __func__, i, cci_clk_info[i].clk_name);
		if (rc < 0) {
			pr_err("%s:%d, failed\n", __func__, __LINE__);
			return rc;
		}
	...
	for (i = 0; i < count; i++) {
		...
		CDBG("%s: clk_rate[%d] = %ld\n", __func__, i, cci_clk_info[i].clk_rate);
	}
	return 0;
}
