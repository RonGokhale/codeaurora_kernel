		...
		u8 *data2, u32 data2_size)
{
	...
	if (unlikely(empty_space <= data1_size + data2_size)) {
		...
		qhdr->qhdr_tx_req =  1;
		dprintk(VPU_ERR,
			"Tx-%d: Insufficient size (%d) to write (%d)\n",
	...
	dprintk(VPU_DBG, "Write Ptr: 0x%08x\n", (u32) write_ptr);

	for (i = 0; i < (data1_size / sizeof(u32)); i++)
		dprintk(VPU_PKT, "Tx%d-pkt: %2d:0x%08x\n",
	...
	if (unlikely(packet_size > filled_space)) {
		...
		if (rxq->cid != VPU_LOGGING_CHANNEL_ID)
			for (i = 0; i < (packet_size / sizeof(u32)); i++)
				dprintk(VPU_PKT, "Rx%d-pkt: %2d:0x%08x\n",
					rxq->cid, i, ((u32 *)(data))[i]);

		/*
		 * to make sure all read/write is done before updating
		 * read index
		 */
		mb();
		...
		u32 total_size)
{
	...
	dprintk(VPU_INFO, "Entering function %s\n", __func__);
...
static void raw_handle_rx_msgs_poll(struct vpu_hfi_device *hdevice)
{
	...
	if (unlikely(!hdevice->handle_msg)) {
		dprintk(VPU_ERR, "No rx handler callback: %p\n", hdevice);
		return;
	}
	...
	for (i = 0; i < VPU_MAX_RXQ_NUM; i++) {
		...
		/*
		 * IPC irq is disabled right now, otherwise need to grab spin
		 * lock
		 */
		do {
			more_data = 0;
...
static int vpu_hfi_boot(struct vpu_hfi_device *hdevice)
{
	...
	if (unlikely(IS_ERR_OR_NULL(subsys))) {
		dprintk(VPU_ERR, "failed to download firmware\n");
		return (int)subsys;
	}
	...
	/* wait for VPU FW up (500 ms) */
	timeout = 25;
	while ((raw_hfi_status_read((u32)(hdevice->reg_base)) & 1) == 0) {
		if (timeout-- <= 0) {
			/* FW bootup timeout */
			dprintk(VPU_ERR, "VPU FW bootup timeout\n");
...
static void _vpu_hfi_watchdog_work_handler(struct work_struct *work)
{
	...
	if (likely(hdevice)) {
		/* notify chan layer, via system channel */
...
irqreturn_t _vpu_hfi_ipc_isr(int irq, void *dev)
{
	...
	dprintk(VPU_ISR, "Entering function %s\n", __func__);
...
int vpu_hfi_init(struct vpu_platform_resources *res)
{
	...
	if (unlikely(!hdevice->main_workq)) {
		dprintk(VPU_ERR, ": create rx workq failed\n");
		rc = -ENOMEM;
		goto workq_fail;
	}
	...
	if (unlikely(!hdevice->reg_base)) {
		dprintk(VPU_ERR,
			"could not map reg addr 0x%x of size 0x%x\n",
			(u32) res->register_base_phy, res->register_size);
		rc = -ENODEV;
		goto error_map_fail1;
	} else
		dprintk(VPU_INFO, "CSR mapped from 0x%08x to 0x%p\n",
				(u32) res->register_base_phy,
				hdevice->reg_base);
	...
	if (unlikely(!hdevice->mem_base)) {
		dprintk(VPU_ERR,
			"could not map mem addr 0x%x of size 0x%x\n",
			(u32) res->mem_base_phy, res->mem_size);
		rc = -ENODEV;
		goto error_map_fail2;
	} else
		dprintk(VPU_INFO, "MEM mapped from 0x%08x to 0x%p\n",
				(u32) res->mem_base_phy,
				hdevice->mem_base);

#else /* VPU_MAPLE_FW_SIM */

	/* allocate the IPC mem, 16 pages */
	hdevice->mem_base =
			(void *) __get_free_pages(GFP_KERNEL, IPC_MEM_ORDER);
	if (unlikely(!hdevice->mem_base)) {
		dprintk(VPU_ERR, "could not allocate IPC mem\n");
		rc = -ENOMEM;
		goto error_map_fail1;
	}

	res->mem_size = PAGE_SIZE * (1 << IPC_MEM_ORDER);
	/* assign phys addr of mem_base to res */
	res->mem_base_phy = __pa(hdevice->mem_base);

	vpu_sim_start(&res->pdev->dev, res->irq, hdevice);
	...
	if (unlikely(rc)) {
		dprintk(VPU_ERR, ": IPC queue init failed\n");
		goto error_hfiq;
	}
...
error_hfiq:
#ifdef VPU_MAPLE_FW_SIM
	vpu_sim_stop(&res->pdev->dev);
	free_pages((unsigned long)(hdevice->mem_base), IPC_MEM_ORDER);
#else
...
#ifdef VPU_MAPLE_FW_SIM
	vpu_sim_stop(&hdevice->platform_resouce->pdev->dev);
	free_pages((unsigned long)(hdevice->mem_base), IPC_MEM_ORDER);
#else
	iounmap(hdevice->reg_base);
	iounmap(hdevice->mem_base);
		...
		hfi_handle_event event_handler)
{
	...
	if (unlikely(rc)) {
		dprintk(VPU_ERR, ": request_irq failed (watchdog irq)\n");
	...
	if (hdevice->irq_no) {
		...
		rc = request_irq(hdevice->irq_no, _vpu_hfi_ipc_isr,
				IRQF_TRIGGER_HIGH, "vpu", hdevice);
		if (unlikely(rc)) {
			dprintk(VPU_ERR, ": request_irq failed (IPC irq)\n");
			goto error_irq2_fail;
		}
	} else {
		/* polling if no IPC interrupt */
		hdevice->use_poll = 1;
		queue_delayed_work(hdevice->main_workq, &hdevice->poll_work,
				msecs_to_jiffies(VPU_POLL_INTERVAL_MS));
	}

	dprintk(VPU_INFO, "interrupts registered\n");
#endif
	return 0;
...
void vpu_hfi_enable(u32 cid, void *priv)
{
	...
	if (likely(cid < VPU_MAX_TXQ_NUM)) {
		...
		/* inc the active q number in qtab */
		if (updated) {
			spin_lock_irqsave(&dev->qlock, flags);
...
void vpu_hfi_disable(u32 cid)
{
	...
	if (likely(cid < VPU_MAX_TXQ_NUM)) {
		...
		/* decrement the active q number in qtab */
		if (updated) {
			spin_lock_irqsave(&dev->qlock, flags);
			dev->qtbl->qtbl_num_active_q--;
			spin_unlock_irqrestore(&dev->qlock, flags);
		...
		u8 *extra_data, u32 extra_size)
{
	...
	if (unlikely(tq->state == HFI_QUEUE_STATE_DISABLED)) {
		...
		dprintk(VPU_ERR, "TX queue not inited, cannot send\n");
		rc = -EPERM;
	} else {
		mutex_lock(&tq->lock);
...
int vpu_hfi_read_log_data(u32 cid, char *buf, int buf_size)
{
	...
	if (unlikely(cid != VPU_LOGGING_CHANNEL_ID)) {
		dprintk(VPU_ERR, "%s must only access the logging queue!\n",
				__func__);
		res = -EACCES;
	...
	do {
		...
		if (max_size <= 0) {
			dprintk(VPU_WARN, "max size reached in %s!\n",
					__func__);
			break;
		}
...
int vpu_hfi_dump_smem_line(char *buf, size_t size, u32 offset)
{
	...
	if (offset >= mem_size) {
		dprintk(VPU_ERR, "offset=%08x > memsize=%08x\n",
				offset, mem_size);
		return -EACCES;
	}
	if (size < (14 + 4 * 11 + 2))
