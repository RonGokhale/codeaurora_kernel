...
int vpu_vb2_flush_buffers(struct vpu_dev_session *session, int port)
{
	...
	/* retrieve any buffers on pending list (not sent to Maple yet) */
	...
	/* return if no buffers were sent to Maple */
	if (!atomic_read(&q->queued_count))
		return 0;

	/* Flush buffers from maple */
	dprintk(VPU_DBG, "%s, Flushing port %d buffers\n", __func__, port);
	ret = vpu_hw_session_flush(session->id, flush_port);
	if (ret)
		dprintk(VPU_ERR, "Critical!!! port %d flush failed\n", port);

	/* Manually retrieve any buffers that maple didn't return */
	for (i = 0; i < q->num_buffers; i++) {
		if (q->bufs[i] && q->bufs[i]->state == VB2_BUF_STATE_ACTIVE) {
			dprintk(VPU_WARN, "Retrieving buffer %d from port %d\n",
				i, get_port_number(q->type));
...
static int vpu_vb2_ops_buf_init(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		...
		if (!vpu_buf->planes[i].mem_cookie) {
			vpu_buf->planes[i].mem_cookie = vpu_iommu_create_handle(
					&session->core->resources);
			if (!vpu_buf->planes[i].mem_cookie) {
				ret = -ENOMEM;
				goto err_buf_init;
			}
		...
		ret = vpu_iommu_map_user_mem(vpu_buf->planes[i].mem_cookie,
					   vpu_buf->planes[i].user_fd,
		...
		if (ret) {
			...
			vpu_buf->planes[i].mapped_address[ADDR_INDEX_VPU] =
			      vpu_iommu_get_addr(vpu_buf->planes[i].mem_cookie);
			vpu_buf->valid_addresses_mask |= ADDR_VALID_VPU;
		}
	}
	return 0;
...
static void vpu_vb2_ops_buf_queue(struct vb2_buffer *vb)
{
	...
	if (session->streaming_state != ALL_STREAMING ||
		 ...
		 * not yet passed to maple, waiting for session to stream on
		...
		if (ret) {
			dprintk(VPU_ERR, "buf_queue fail, returning buffer\n");
...
static void vpu_vb2_ops_buf_cleanup(struct vb2_buffer *vb)
{
	...
	dprintk(VPU_DBG, "%s, unmap buffer #%d from iommu\n", __func__,
			vb->v4l2_buf.index);
	vpu_buf = to_vpu_buffer(vb);

	for (i = 0; i < vb->num_planes; i++) {
		if (vpu_buf->planes[i].mem_cookie)
			vpu_iommu_destroy_handle(vpu_buf->planes[i].mem_cookie);
...
static int vpu_vb2_ops_stop_streaming(struct vb2_queue *q)
{
	...
	dprintk(VPU_DBG, "%s called for port %d\n", __func__, port);

	/* Stop end-to-end session streaming on first streamoff */
	if (session->streaming_state == ALL_STREAMING) {
		if (vpu_hw_session_stop(session->id))
			dprintk(VPU_ERR, "Critical!!! chanstop failed, %s\n",
					__func__);
		dprintk(VPU_DBG, "Session streaming stopped\n");
	}
			...
			void *pdata)
{
	...
	if (ret < 0)
		dprintk(VPU_ERR, "%s:init vb2 queue (type = %d) fails\n",
			__func__, type);
