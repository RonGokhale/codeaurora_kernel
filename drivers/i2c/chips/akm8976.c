...
#ifdef CONFIG_ANDROID_POWER
static void akm8976_early_suspend(android_early_suspend_t *handler)
{
...
static void akm8976_early_resume(android_early_suspend_t *handler)
{
	enable_irq(this_client->irq);
	if (atomic_read(&open_flag) == 2)
...
int akm8976_probe(struct i2c_client *client)
{
...
#ifdef CONFIG_ANDROID_POWER
	akm->early_suspend.suspend = akm8976_early_suspend;
	akm->early_suspend.resume = akm8976_early_resume;
