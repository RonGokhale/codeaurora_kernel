...
static int toshiba_haps_protection_level(acpi_handle handle, int level)
{
	...
	pr_info("HDD protection level: %d\n", level);
...
static ssize_t haps_movement_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct toshiba_haps_dev *haps = dev_get_drvdata(dev);
...
static DEVICE_ATTR(movement, S_IRUGO, haps_movement_show, NULL);
...
{
	struct toshiba_haps_dev *haps = acpi_driver_data(device);

	if (event == 0x80) {
		pr_info("Movement detected\n");
		haps->movement = 1;
	} else if (event == 0x81) {
		pr_info("Movement stabilized\n");
		haps->movement = 0;
	}
}
