...
static void xadc_zynq_write_reg(struct xadc *xadc, unsigned int reg,
	uint32_t val)
{
	writel(val, xadc->base + reg);
}

static void xadc_zynq_read_reg(struct xadc *xadc, unsigned int reg,
	uint32_t *val)
{
	...
	unsigned int n)
{
	...
	for (i = 0; i < n; i++)
		xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_CFIFO, cmd[i]);
}
...
static void xadc_zynq_drain_fifo(struct xadc *xadc)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_STATUS, &status);

	while (!(status & XADC_ZYNQ_STATUS_DFIFOE)) {
		xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_DFIFO, &tmp);
		xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_STATUS, &status);
	}
}
	...
	unsigned int val)
{
	...
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_INTMSK,
	...
	uint16_t val)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_CFG, &tmp);
	tmp &= ~XADC_ZYNQ_CFG_DFIFOTH_MASK;
	tmp |= 0 << XADC_ZYNQ_CFG_DFIFOTH_OFFSET;
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_CFG, tmp);
	...
	uint16_t *val)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_DFIFO, &resp);
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_DFIFO, &resp);
...
static void xadc_zynq_unmask_worker(struct work_struct *work)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_STATUS, &misc_sts);
	...
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_INTSTS, unmask);
...
static irqreturn_t xadc_zynq_interrupt_handler(int irq, void *devid)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_INTSTS, &status);
	...
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_INTSTS, status);
	...
	struct iio_dev *indio_dev, int irq)
{
	...
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_CTL, XADC_ZYNQ_CTL_RESET);
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_CTL, 0);
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_INTSTS, ~0);
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_INTMSK, xadc->zynq_intmask);
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_CFG, XADC_ZYNQ_CFG_ENABLE |
...
static unsigned long xadc_zynq_get_dclk_rate(struct xadc *xadc)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_CFG, &val);
...
static void xadc_zynq_update_alarm(struct xadc *xadc, unsigned int alarm)
{
	...
	xadc_zynq_read_reg(xadc, XADC_ZYNQ_REG_INTSTS, &status);
	xadc_zynq_write_reg(xadc, XADC_ZYNQ_REG_INTSTS, status & alarm);
	...
	struct iio_chan_spec const *chan, int *val, int *val2, long info)
{
	...
	case IIO_CHAN_INFO_SAMP_FREQ:
		ret = xadc_read_reg(xadc, XADC_REG_CONF2, &val16);
		if (ret)
			return ret;
	...
	struct iio_chan_spec const *chan, int val, int val2, long info)
{
	...
	return xadc_update_reg(xadc, XADC_REG_CONF2, XADC_CONF2_DIV_MASK,
...
static int xadc_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < 16; i++) {
		...
		if (i % 8 < 4 || i == 7)
			...
			xadc->threshold[i] = 0;
		xadc_write_reg(xadc, XADC_REG_THRESHOLD(i), xadc->threshold[i]);
	}
