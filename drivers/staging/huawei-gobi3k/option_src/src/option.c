...
#define DRIVER_VERSION "v0.7.2.2"
...
#define GAOTONG_VENDOR                          0x05c6 
#define GAOTONG_PRODUCT                         0x920d
...
#define HUAWEI_PRODUCT_14F1			0x14F1
#define HUAWEI_PRODUCT_14F2			0x14F2
#define HUAWEI_PRODUCT_14F3			0x14F3
#define HUAWEI_PRODUCT_14F4			0x14F4
#define HUAWEI_PRODUCT_14F5			0x14F5
...
static struct usb_device_id option_ids[] = {
	...
	{ USB_DEVICE(GAOTONG_VENDOR, GAOTONG_PRODUCT)},
	{ USB_DEVICE(HUAWEI_VENDOR_ID, HUAWEI_PRODUCT_14F1) },
	{ USB_DEVICE(HUAWEI_VENDOR_ID, HUAWEI_PRODUCT_14F2) },
	{ USB_DEVICE(HUAWEI_VENDOR_ID, HUAWEI_PRODUCT_14F3) },
	{ USB_DEVICE(HUAWEI_VENDOR_ID, HUAWEI_PRODUCT_14F4) },
	{ USB_DEVICE(HUAWEI_VENDOR_ID, HUAWEI_PRODUCT_14F5) },
	{ } /* Terminating entry */
};
...
int Isgobimodule(struct usb_serial *serial)
{
    int i = 0;
    int gobiproduct[] = {HUAWEI_PRODUCT_14F1,HUAWEI_PRODUCT_14F2,HUAWEI_PRODUCT_14F3,HUAWEI_PRODUCT_14F4,HUAWEI_PRODUCT_14F5};
    int gobicount = sizeof(gobiproduct)/sizeof(int);
    if (serial->dev->descriptor.idVendor == GAOTONG_VENDOR && serial->dev->descriptor.idProduct == GAOTONG_PRODUCT)
    {	

        return 1; //gobi module

    }	
    if (serial->dev->descriptor.idVendor == HUAWEI_VENDOR_ID)
    {
        for(i = 0; i < gobicount;i++)
        {
            if (serial->dev->descriptor.idProduct == gobiproduct[i])
            {
			...
			const struct usb_device_id *id)
{
	...
	if (serial->dev->descriptor.idVendor == HUAWEI_VENDOR_ID 
		|| (serial->dev->descriptor.idVendor == GAOTONG_VENDOR && serial->dev->descriptor.idProduct == GAOTONG_PRODUCT)) {
...
void startGPS(struct usb_serial_port* port)
{
	const char startMessage[] = "$GPS_START";
	int nResult;
	int bytesWrote;
	nResult = usb_bulk_msg(port->serial->dev,usb_sndbulkpipe(port->serial->dev,port->bulk_out_endpointAddress),(void*)&startMessage[0],sizeof(startMessage),&bytesWrote,1000);
	if(0 != nResult)
	{
		printk(KERN_ERR"%s marui usb_bulk_msg error\n",__func__);
	}	 
	if(bytesWrote != sizeof(startMessage))
	{
		printk(KERN_ERR"%s bytesWrote != sizeof(startMessage)\n",__func__);
	}
}

void stopGPS(struct usb_serial_port* port)
{
	const char startMessage[] = "$GPS_STOP";
	int nResult;
	int bytesWrote;
	nResult = usb_bulk_msg(port->serial->dev,usb_sndbulkpipe(port->serial->dev,port->bulk_out_endpointAddress),(void*)&startMessage[0],sizeof(startMessage),&bytesWrote,1000);
	if(0 != nResult)
...
static int option_open(struct tty_struct *tty, struct usb_serial_port *port)
{
	...
	intfdata = serial->private;
        ...
        if (serial->dev->descriptor.idVendor == GAOTONG_VENDOR && serial->dev->descriptor.idProduct == GAOTONG_PRODUCT)
       	{	
		if(serial->interface->cur_altsetting->desc.bInterfaceNumber == 3)
        	{
                    startGPS(port);
        	}
	}	
        ...
        if (serial->dev->descriptor.idVendor == HUAWEI_VENDOR_ID)
        {
            if (serial->dev->descriptor.idProduct == HUAWEI_PRODUCT_14F1 ||
               serial->dev->descriptor.idProduct == HUAWEI_PRODUCT_14F2 ||
               serial->dev->descriptor.idProduct == HUAWEI_PRODUCT_14F3 ||
               serial->dev->descriptor.idProduct == HUAWEI_PRODUCT_14F4)
            {
                if (serial->interface->cur_altsetting->desc.bInterfaceNumber == 3)
                {
                    startGPS(port);
            ...
            if (serial->dev->descriptor.idProduct == HUAWEI_PRODUCT_14F5)
            {
