...
#include "cdc_encap.h"
/////////////////////////////////////////////////////////////////////////////////////////////////
#define DRIVER_VERSION "v2.05.00.00"
...
static void rx_complete (struct urb *urb)
{
	...
	{
		if (netif_running(dev->net)){
			printk(KERN_ERR"fxz-%s: called status[%d]\n", __func__, urb_status);
...
static int hw_start_xmit (struct sk_buff *skb, struct net_device *net)
{
	...
	/*Begin: added for selective suspend by lkf*/
	retval = usb_autopm_get_interface_async(dev->intf);
	if (0 > retval) {
		goto drop;	
	}
	/*End: added for selective suspend by lkf*/
...
//	DECLARE_MAC_BUF(mac);
	...
	/*In Android, set the carrier on as default*/
	...
	//Begin: Add the encapsulation driver to create the device file in /dev
	{
		struct cdc_encap	*encap;
		status = cdc_encap_init(net->name, udev, &encap);
		if (status < 0) {
			printk(KERN_ERR"fxz-%s: failed to init encapsulation command\n", __func__);
			return 0;
		}
		dev->driver_priv = encap;
		printk(KERN_ERR"fxz-%s: called\n", __func__);
	}
	//End: Add the encapsulation driver to create the device file in /dev
	return 0;
...
bool gobidevice = false;
void isgobidevice(int idProduct,int idVendor)
{
    int i =0;
    int gobipid[] = { 0x14f1, 0x14f2, 0x14f3, 0x14f4, 0x14f5 };//Huawei gobi device PID 
    if(0x920D == idProduct && 0x05c6 == idVendor)
    {//GOBI VOID AND PID
        gobidevice = true;
    }
    for(i = 0;i < sizeof(gobipid)/sizeof(int);i++)
    {
        if(idProduct == gobipid[i])
        {    
            gobidevice = true;
        }
    }
...
static int hw_cdc_bind(struct hw_cdc_net *dev, struct usb_interface *intf)
{
    ...
    isgobidevice(dev->udev->descriptor.idProduct,dev->udev->descriptor.idVendor);
	...
	if (sizeof dev->data < sizeof *info)
   	...
   	if(true == gobidevice)
   	{
        if ((intf->cur_altsetting->desc.bInterfaceNumber != 0)&&(intf->cur_altsetting->desc.bInterfaceNumber != 5))
        {
            return -ENODEV;
        }
   	} 
			...
			void *buf)
{
	...
	printk(KERN_ERR"fxz-%s:called\n", __func__);
	/*Begin: added for selective suspend by lkf*/
	ret = usb_autopm_get_interface_async(intf);
	if (0 > ret) {
		goto op_error;	
	}
	/*End: added for selective suspend by lkf*/
