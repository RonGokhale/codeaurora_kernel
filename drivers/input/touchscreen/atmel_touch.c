...
struct mxt_data {
	...
	const struct autoplat001_mxt_platform_data *pdata;
...
#define PANEL_MAX_X	800
#define PANEL_MAX_Y	480
#define PANEL_MIN_XY	0

#define CENTER_POINT1_X	237
#define CENTER_POINT1_Y	378

#define CENTER_POINT2_X	567
#define CENTER_POINT2_Y	378
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (!data->pdata->config_file)
		return 0; /* no define config file ,return 0 */
...
{
	const struct autoplat001_mxt_platform_data *pdata = client->dev.platform_data;
	...
	if (pdata->init_platform_hw) {
		...
		if (error)
			printk("[TSP] init platform data issue\n");
	}
