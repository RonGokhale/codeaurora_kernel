...
static char *_depc;
static unsigned int _depc_size;
static unsigned int _c_bl[CB_COUNT][CBD_COUNT];
static unsigned int _device_primary;
static unsigned int _device_all;
/* ION states */
...
static int _get_dev_mask_for_pid(int port_id)
{
	switch (port_id) {
	case SLIMBUS_0_RX:
...
static int _get_pid_from_dev(int device)
{
...
static int _get_cb_for_dev(int device)
{
	int i;
...
int msm_dts_eagle_set_volume(struct audio_client *ac, int lgain, int rgain)
{
	...
	for (i = 0; i < vol_cmd_cnt; i++) {
		if (vol_cmds_d[i].d[0] & 0x8000) {
			...
			vol_cmds[i][idx] = clamp((int)(((long long)val *
						vol_cmds_d[i].d[1]) >> 16),
						vol_cmds_d[i].d[2],
						vol_cmds_d[i].d[3]);
		...
		if (idx < 0) {
			pr_err("DTS_EAGLE_DRIVER_VOLUME: no cache found for primary device %i.\n",
				_device_primary);
			return -EINVAL;
		}
...
DTS_EAGLE_IOCTL_GET_PARAM_EXIT:
	...
	case DTS_EAGLE_IOCTL_SET_PARAM: {
		struct dts_eagle_param_desc depd;
		int offset = 0, just_set_cache = 0, tgt;
	...
	case DTS_EAGLE_IOCTL_SET_CACHE_BLOCK: {
		unsigned int b_[CBD_COUNT+1], *b = &b_[1], cb;
		...
		if (copy_from_user((void *)b_, (void *)arg, sizeof(b_))) {
			pr_err("DTS_EAGLE_DRIVER_POST: error copying cache block data\n");
			return -EFAULT;
		}
		cb = b_[0];
		if (cb >= CB_COUNT) {
	...
	case DTS_EAGLE_IOCTL_SET_ACTIVE_DEVICE: {
		...
		if (copy_from_user((void *)data, (void *)arg, sizeof(data))) {
			pr_err("DTS_EAGLE_DRIVER_POST: error copying active device data\n");
			return -EFAULT;
		}
		if (data[1] != 0) {
