...
#if 0
static int snd_pcm_open_fd(int fd, struct alsa_play_data *play_data){
	struct file *file;
	struct inode *inode;
	struct snd_pcm  *pcm;
	unsigned int minor;
	...
	minor = iminor(inode);
	if(!(pcm = snd_lookup_minor_data(minor, SNDRV_DEVICE_TYPE_PCM_PLAYBACK))) {
		fput(file);
		return -EBADFD;
	}

	play_data->file = file;
	play_data->inode = inode;
	play_data->pcm = pcm;
	return 0;
}
#endif

static int snd_pcm_open_by_minor(unsigned int minor, struct alsa_play_data *play_data)
{
	struct snd_pcm  *pcm;
	if(!(pcm = snd_lookup_minor_data(minor, SNDRV_DEVICE_TYPE_PCM_PLAYBACK))) {
		return -EBADFD;
...
static void snd_mixer_config_widget(struct alsa_play_data *play_data, int numid, int i)
{
	...
	if (info == NULL || ctl == NULL) {
		goto bad;
	}
	if (kctl->info(kctl, info)) {
		goto bad;
	}

	if (kctl->get(kctl, ctl)) {
		goto bad;
	}

	ctl->value.integer.value[0] = i;

	if ((kctl->put(kctl, ctl)) < 0) {
		goto bad;
	}
bad:
	up_read(&card->controls_rwsem);
...
static int snd_pcm_open_stream (struct alsa_play_data *play_data)
{
	...
	//fmode_t f_mode = play_data->file->f_mode;
	...
	for (idx = 0; idx < 2; idx++)
	{
		...
		if (idx != SNDRV_PCM_STREAM_PLAYBACK)
			continue;
		/* TODO: do not need to check file permission, dummy file will be used here
		if (idx == SNDRV_PCM_STREAM_PLAYBACK) {
			if(!(f_mode & FMODE_WRITE))
				continue;
		}*/
		err = snd_pcm_open_substream(play_data->pcm, idx, play_data->dummy_file, &substream);
		if(err < 0)
			goto free_buffer;
...
static int snd_pcm_set_params(struct snd_pcm_substream *substream)
{
	...
	if ((err = snd_pcm_kernel_ioctl(substream, SNDRV_PCM_IOCTL_HW_PARAMS, hw_params)) < 0)
	{
...
static int snd_pcm_write_file(struct snd_pcm_substream *substream, char *ptr, int count)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	int remaining = count;
	...
	while (remaining) {
		...
		//send_size = bytes_to_frames(runtime, size);
		send_size = size / 4;

		fs = snd_enter_user();
		ret = snd_pcm_lib_write(substream, (void __force __user *)pos, send_size);
		snd_leave_user(fs);
		if (ret < 0)
			break;
...
static int playback_thread(void *data)
{
	...
	for(;;) {
		rc = wait_event_interruptible(play_data->w_wait, play_data->valid == 1);
		if (rc < 0)
			break;

		if (kthread_should_stop())
			break;
		pr_info("start notification sound play\n");
		rc = snd_pcm_write_file(play_data->substream, play_data->buffer, play_data->count);
...
static int __init alsa_play_init(void)
{
	...
	//snd_mixer_config_widget(play_data, ALSA_MIXER_NUMID, 1);
	...
	play_data->playback_thread = kthread_create(playback_thread, (void *)play_data, "alsa_play");
